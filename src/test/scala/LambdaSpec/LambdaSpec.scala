...
//  eventType: String = "FastlyDebug")
  "Lambda Application" should {
    ...
    "correctly parse a valid log entry" in {
      val record = "<134>2016-01-08T18:35:59Z cache-atl6234 AmazonS3[168183]: 1.2.3.4 Fri, 08 Jan 2016 18:35:59 GMT GET /path/hello.txt www.domain.com 200 HIT, HIT (null)"
      val result = parseRecord(record)
      result mustNotEqual None
      result.get.hostname mustEqual "www.domain.com"
      result.get.hitMiss mustEqual "HIT"
    ...
    "correctly parse all formats of HIT/MISS" in {
      ...
      res1 mustNotEqual None and(res1.get.hitMiss mustEqual "HIT")

      val res2 = parseRecord(record2)
      res2 mustNotEqual None and(res2.get.hitMiss mustEqual "HIT")

      val res3 = parseRecord(record3)
      res3 mustNotEqual None and(res3.get.hitMiss mustEqual "MISS")

      val res4 = parseRecord(record4)
      res4 mustNotEqual None and(res4.get.hitMiss mustEqual "MISS")

      val res5 = parseRecord(record5)
      res5 mustNotEqual None and(res5.get.hitMiss mustEqual "HIT")

      val res6 = parseRecord(record6)
      res6 mustNotEqual None and(res6.get.hitMiss mustEqual "MISS")
